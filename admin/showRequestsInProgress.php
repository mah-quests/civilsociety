        ...
        <div class="page-wrapper">
            ...
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h3 class="text-secondary">List of all household surveys done by the Civil Society </h3>
                </div>
...
<?php
    ...
    if ($input == "food"){
        $column = "access_to_food";
    }elseif ($input == "jobs"){
        $column = "job";
    }elseif ($input == "electricity"){
        $column = "electricity";
    }elseif ($input == "medic"){
        $column = "medication";
    }elseif ($input == "identitydoc"){
        $column = "identity_documents";
    } elseif ($input == "blankets"){
        $column = "clothes_blankes";
    } elseif ($input == "internet"){
        $column = "data_internet";
    }

    $input = $_GET['id'];

    $sql="select * from users_orders where unique_code in (select unique_code from assigned_tasks where $column='yes')";

    $query=mysqli_query($db,$sql);

    if(!mysqli_num_rows($query) > 0 ){
        echo '<td colspan="8"><center>No Requests!</center></td>';
    } else {
