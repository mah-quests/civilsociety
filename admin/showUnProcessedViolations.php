...
<?php
    $sql="SELECT users.*, users_orders.* FROM users INNER JOIN users_orders ON users.u_id=users_orders.u_id where users_orders.status is null and users_orders.unique_code in (SELECT unique_code FROM request_violations where  any_kind_abuse='Yes' )";
