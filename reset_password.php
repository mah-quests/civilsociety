...
if(isset($_POST['submit']))   // if button is submit
{
  ...
  if(!empty($_POST["submit"]))   // if records were not empty
     {
    $loginquery ="SELECT * FROM users WHERE username='$username'"; // Check if the username is in the database
    ...
    $success = "User found... You will now reset your password! <p>Redirection in <span id='counter'>5</span> second(s).</p>
                            <script type='text/javascript'>
                            function countdown() {
                              var i = document.getElementById('counter');
                              if (parseInt(i.innerHTML)<=0) {
                                location.href = 'new_password?$user_id'.php';
                              }
     ...
     header("refresh:5;url=new_password.php?user_id=$user_id");
      } 
      else
      {
      ...
      <div class="card-body">
        ...
        <form  action="" method="post">
          <div class="input-group form-group">
            ...
            <input type="text" class="form-control" placeholder="Enter your Identification Number" name="username" required=>            
          </div>
          <div class="form-group">
            <input type="submit" id="buttn" name="submit" value="Login" class="btn float-right login_btn">
          </div>
        </form>
      </div>
      <div align="center">
